...
class Like(db.Model):
    __tablename__ = 'likes'
    id: Mapped[int] = mapped_column(primary_key=True)
    liker_id: Mapped[int] = mapped_column(ForeignKey('users.id', ondelete='CASCADE'), nullable=False)
    liked_id: Mapped[int] = mapped_column(ForeignKey('users.id', ondelete='CASCADE'), nullable=False)
    created_at: Mapped[DateTime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), nullable=False
    )

    # Relaciones
    liker: Mapped[User] = relationship('User', foreign_keys=[liker_id], back_populates='likes_given')
    liked: Mapped[User] = relationship('User', foreign_keys=[liked_id], back_populates='likes_received')
...
class Match(db.Model):
    ...
    # Relaciones
    user1: Mapped[User] = relationship('User', foreign_keys=[user1_id], back_populates='matches_initiated')
    user2: Mapped[User] = relationship('User', foreign_keys=[user2_id], back_populates='matches_received')
...
class Reject(db.Model):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    rejector_id: Mapped[int] = mapped_column(ForeignKey('users.id', ondelete='CASCADE'), nullable=False)
    rejected_id: Mapped[int] = mapped_column(ForeignKey('users.id', ondelete='CASCADE'), nullable=False)
    created_at: Mapped[DateTime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), nullable=False
    )
